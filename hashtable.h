...
// Реализуем узел в виде объекта с типом ячейки [DELETED, KEY, EMPTY] - возможные состояния
enum TypeOfNode {
    d, // deleted
    k, // key
    e, // empty/nil
...
public:
    ...
    bool Has(const T &key) {
        ...
        for (int i = 0; i < table.size(); ++i) {
            hash = (hash + i) % table.size();  // Пробирование
            switch (table[hash].nodeType) {
                case TypeOfNode::e:
                    return false;
                case TypeOfNode::d:
    ...
    bool Delete(const T &key) {
        ...
        if (delete_nodes >= HASH_TABLE_MAX_DELETE_FILL * table.size()) {
            DDNodes();  // Delete Deleted Nodes DDN
        }
    ...
    // Вызывая эту функцию в SetGraph, мы будем получать вектор вершин-потомков
...
// Можно не писать собственный Hasher, так как используется по умолчанию библиотечный
//class Int64tHasher {
//public:
//    explicit Int64tHasher(const uint32_t &m_prime = 71) : prime(m_prime) {}
//
//    size_t operator()(const std::uint32_t &key) const {
//        // Метод умножения
//        const size_t gold_const = 2654435769;
//        const size_t slice = 16;
//        return (k * gold_const % static_cast<size_t>(pow(2, 32))) >> slice;
//    }
//
//private:
//    uint32_t prime;
